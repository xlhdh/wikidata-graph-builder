(function() {
  ...
  insertData = function(graph, data, activeItem, mode, sizeLogScale) {
    var arrowOffset, charge, childrenForce, circle, container, drag_rect, dragended, dragged, dragstarted, fix1, hasSize, height, levelingForce, line, linkDistance, links, marker, maxSize, minSize, node, nodeid, nodes, path, radius, radscaler, ref, resize, scaleRange, simulation, svg, svg_group, text, tick, tooltip, tooltipFn, transform, useLogScale, width, zoom, zoomed;
    ...
    levelingForce = function(alpha) {
      var k;
      k = 6 * alpha;
      return line.each(function(d) {
        var source, target;
        source = d.source, target = d.target;
        if ((d.target.y - d.source.y) > 100) {
          d.source.vy += k;
          return d.target.vy -= k;
        } else {
          d.source.vy -= k;
          return d.target.vy += k;
        }
    ...
    tick = function() {
      var diff, div, length, prod, scale, sum;
      line.each(function(d) {
        var source, target;
        source = d.source, target = d.target;
        if (source.fy && !target.fy) {
          target.fy = source.fy + linkDistance;
        }
        if (target.fy && !source.fy) {
          return source.fy = target.fy - linkDistance;
        }
      });
      if (hasSize) {
        length = function(arg) {
        ...
        line.attr('x1', function(arg) {
          var sp;
          sp = arg.sp;
          return source.x;
        });
        line.attr('y1', function(arg) {
          var sp;
          sp = arg.sp;
          return source.y;
        });
        line.attr('x2', function(arg) {
          var sp;
          sp = arg.sp;
          return target.x;
        });
        line.attr('y2', function(arg) {
          var sp;
          sp = arg.sp;
          return target.y;
        });
    ...
    marker = svg.append('defs').selectAll('marker').data(['direction']).enter().append('marker');
    ...
    line = container.append('g').selectAll('line').data(links).enter().append('line');
    line.attr('marker-end', 'url(#direction)');
    radius = hasSize ? (function(d) {
      return d.radius;
    }) : 6;
    circle = container.append('g').selectAll('circle').data(simulation.nodes()).enter().append('circle');
    ...
    circle.call(d3.drag().on("start", dragstarted).on("drag", dragged).on("end", dragended));
    text = container.append('g').selectAll('text').data(simulation.nodes()).enter().append('text').text(function(d) {
      return d.name;
    }).on('click', function(o) {
      window.open(o.url);
    });
...
(function() {
  ...
  WikiTools = angular.module('WikiTools', []).controller('AuthorizeController', ["$sce"]);
  ...
  WikiToolsService = function($log, $http, $httpParamSerializer) {
    ...
    this.searchEntities = (function(_this) {
      return function(type, query, language) {
        ...
        return _this.get(_this.wikidata, params).then(success, error)["catch"](angular.noop);
      };
    })(this);
